...
class GcpLoggingPino {
     ...
     * string or is empty.
     */
    async initializeOptionsAsync(options) {
        if (options?.serviceContext) {
            ...
            const serviceContext = await (0, logging_1.detectServiceContext)(cloudLog.auth);
    ...
    getLoggingClient() {
        if (!this.cachedLoggingClient) {
            this.cachedLoggingClient = new logging_1.Logging({ auth: this.auth });
